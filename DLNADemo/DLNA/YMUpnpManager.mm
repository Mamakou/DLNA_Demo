...
- (void)startDMServer
{
    ...
    [[UPnPEngine getEngine] intendStartItunesMusicServerWithServerName:[UIDevice currentDevice].name];
...
- (void)startWebServer
{
    ...
    [self.webServer addHandlerForMethod:@"NOTIFY" pathRegex:@"/dlna/callback" requestClass:GCDWebServerDataRequest.self processBlock:^GCDWebServerResponse *(__kindof GCDWebServerRequest *request) {
        if([request isKindOfClass:[GCDWebServerDataRequest class]]){
            GCDWebServerDataRequest *re = (GCDWebServerDataRequest*)request;
            ///数据分析
            [weakself parseWebServerMessage:re.data];
        }
        return [GCDWebServerDataResponse responseWithHTML:@"<html><body><p>Hello World</p></body></html>"];
    }];
    
    [self.webServer startWithPort:5000 bonjourName:nil];
...
#warning 这里只是个参考 具体需要获取什么信息，通过当前测试的设备进行对应解析
/**
 真实项目案例中提供的解析参考
 */
- (void)demoParseWebserverResponseData:(NSData*)data
{
    ...
    NSArray *xmlArray = [xmlEle children];
    GDataXMLElement *element1 = [xmlArray objectAtIndex:0];
    NSArray *xmlArray1 = [element1 children];
    
    GDataXMLElement *element2 = [xmlArray1 objectAtIndex:0];
    NSArray *xmlArray2 = [element2 children];
    
    GDataXMLElement *element3 = [xmlArray2 objectAtIndex:0];
    
    ////这里是真实案例中的设备返回的，只作为参考
    if ([[element3 name] isEqualToString:@"text"]) {
        GDataXMLDocument *xmlDoc1 = [[GDataXMLDocument alloc] initWithXMLString:[NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"utf-8\"?>%@",[element3 stringValue]] options:0 error:nil];
        GDataXMLElement *xmlEle22 = [xmlDoc1 rootElement];
        NSArray *users = [xmlEle22 elementsForName:@"InstanceID"];
        
        for (GDataXMLElement *user in users) {
            NSString *userId = [[user attributeForName:@"val"] stringValue];
            NSLog(@"InstanceID id is:%@",userId);
            
            NSArray *xmlArray5 = [user children];
            
            for (GDataXMLElement *user33 in xmlArray5 ) {
                if ([[user33 name] isEqualToString:@"TransportState"]) {
                    NSLog(@"TransportState:%@", [[user33 attributeForName:@"val"] stringValue]);
                    self.TransportState = [[user33 attributeForName:@"val"] stringValue];
                }
                
                //                        else if ([[user33 name] isEqualToString:@"Volume"]){
                //                            NSLog(@"55556666665545Volume:%@", [[user33 attributeForName:@"val"] stringValue]);
                //                        }else if ([[user33 name] isEqualToString:@"Mute"]){
                //                            NSLog(@"55556666665545Mute:%@", [[user33 attributeForName:@"val"] stringValue]);
                //                        }else if ([[user33 name] isEqualToString:@"AbsoluteTimePosition"]){
                //                            NSLog(@"55556666665545AbsoluteTimePosition:%@", [[user33 attributeForName:@"val"] stringValue]);
                //                        }else if ([[user33 name] isEqualToString:@"CurrentPlayMode"]){
                //                            NSLog(@"55556666665545CurrentPlayMode:%@", [[user33 attributeForName:@"val"] stringValue]);
                //                        }else if ([[user33 name] isEqualToString:@"CurrentMediaDuration"]){
                //                            NSLog(@"55556666665545CurrentMediaDuration:%@", [[user33 attributeForName:@"val"] stringValue]);
                //                        }else if ([[user33 name] isEqualToString:@"CurrentTrackMetaData"]){
                //                            NSLog(@"55556666665545CurrentTrackMetaData:%@", [[user33 attributeForName:@"val"] stringValue]);
                //                        }else if ([[user33 name] isEqualToString:@"CurrentTrackURI"]){
                //                            NSLog(@"55556666665545CurrentTrackURI:%@", [[user33 attributeForName:@"val"] stringValue]);
                //                        }else if ([[user33 name] isEqualToString:@"CurrentMediaDuration"]){
                //                            NSLog(@"55556666665545CurrentMediaDuration:%@", [[user33 attributeForName:@"val"] stringValue]);
                //                        }else if ([[user33 name] isEqualToString:@"CurrentMediaDuration"]){
                //                            NSLog(@"55556666665545CurrentMediaDuration:%@", [[user33 attributeForName:@"val"] stringValue]);
