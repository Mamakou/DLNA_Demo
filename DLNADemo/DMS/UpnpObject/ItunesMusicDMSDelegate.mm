...
- (NSString *)getDocumentPath {
    return [[UPNPUtil sharedInstance] getDocumentPath];
}
...
- (NPT_Result)onBrowseDirectChildren:(PLT_MediaServerBrowseCapsule*)info
{
    ...
    if ([info.objectId isEqualToString:@"0"]) {
        ...
        for (int i = 0; i < [self.albumsArray count]; i++) {
            ...
            MPMediaItemCollection *album = [self.albumsArray objectAtIndex:i];
        ...
        MPMediaItemCollection *album = [self.albumsArray objectAtIndex:[albIndex intValue]];
...
- (NPT_Result)onFileRequest:(PLT_MediaServerFileRequestCapsule*)info withURL:(NSString *)theURL {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"DLNAMDSACCESSTORESOURCES" object:nil];
...
#warning
    ...
    if (entity) entity->SetContentType("video/mp4");// this is for m4a
